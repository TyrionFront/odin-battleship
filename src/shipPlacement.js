...
class ShipPlacement {
  ...
  #placeCurrent(row, col) {
    ...
    this.#placements.push({ length, coordinates: [row, col], direction: this.#direction });
    ...
    this.#updateInfo();
    this.#domController.renderPlacementBoard(this.#boardEl, this.#tempBoard);
    return true;
  }
  ...
  #bindEvents() {
    ...
    this.#boardEl.addEventListener("mouseover", (e) => {
      ...
      if (!cell) return;
      this.#showPreview(parseInt(cell.dataset.row), parseInt(cell.dataset.col));
    }, { signal });

    // Leave: clear the preview when the cursor exits the board
    ...
    this.#boardEl.addEventListener("click", (e) => {
      const cell = e.target.closest(".cell");
      if (!cell) return;
      this.#placeCurrent(parseInt(cell.dataset.row), parseInt(cell.dataset.col));
    }, { signal });
  }
  ...
  #updateInfo() {
    ...
    if (this.#infoEl && ship) {
      this.#infoEl.textContent = `Placing: ${ship.name} (length ${ship.length}) — Direction: ${this.#direction}`;
    }
  }
  ...
  // Flips the placement direction and refreshes the info bar
  toggleDirection() {
